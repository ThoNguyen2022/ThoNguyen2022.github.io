function toggleCreateTaskForm() {
	var formContainer = document.getElementById("createTask");
	formContainer.style.display = (formContainer.style.display === "none") ? "block" : "none";
}
function toggleviewTable(){
	var formContainer = document.getElementById("viewTable");
	formContainer.style.display = (formContainer.style.display === "none") ? "block" : "none";
}
...
function addTask(input) {
	...
	if (currentValue.includes(input)) {
		// Nếu đã có trong input, bỏ chọn (loại bỏ)
		// Xóa giá trị và dọn dẹp dấu phẩy
		taskInput.value = currentValue.split(', ')
									  .filter(task => task !== input)
									  .join(', ')
									  .replace(/,\s*$/, ''); // Xóa dấu phẩy ở cuối
	} else {
		// Nếu chưa có, cộng thêm vào input
		taskInput.value = currentValue ? currentValue + ', ' + input : input;
	}
}

function selectMachine(machine) {
	var machineInput = document.getElementById("machineName");
	machineInput.value = machine; // Gán giá trị máy được chọn vào input
}

function handlePrice(value) {
    // Lấy giá trị hiện tại của input #price
    let currentPrice = parseInt(document.getElementById('price').value);
    
    // Nếu value là số (+ hoặc -), cộng hoặc trừ vào giá trị hiện tại
    if (value.startsWith('+')) {
        currentPrice += parseInt(value); // Thêm giá trị
    } else if (value.startsWith('-')) {
        currentPrice -= parseInt(value); // Trừ giá trị nếu có button trừ (nếu cần)
    } else {
        currentPrice = parseInt(value); // Gán giá trị trực tiếp
    }

    // Cập nhật lại input với giá trị mới
...
function loadDataReport(start, end, status) {
	{
		const taskListRef = database.ref('tasks');
		taskListRef.once('value', function (snapshot) {
			completedTask= [];
			...
			snapshot.forEach(function (childSnapshot) {
				...
				} else if (task.StatusName === "Đang làm")
				{
					inprogressTask.push(task);
				}else if (task.StatusName === "Chưa làm")
				{
					newTask.push(task);
				}
				taskList.push(newTask);
				taskList.push(inprogressTask);
